...
class SoftOptionCriticConv(nn.Module):
    def __init__(self,
                in_features,
                num_actions,
                num_options,
        ...
        super(SoftOptionCriticConv, self).__init__()
        ...
        # Temperature parameter for soft actor-critic, Target entropy is Random policy entropy
        # Log alpha is learnable as in Soft Actor-Critic Algorithms and Applications (Haarnoja et al)
        self.target_entropy = -np.log(1.0 / num_actions) * 0.98  # target entropy trick
    ...
    def get_action(self, state, option):
        #Handle Batch Logic (for Critic Loss) vs Single Item (for Rollout)
        if isinstance(option, torch.Tensor):
            # Batch Mode: (Batch, Features) @ (Batch, Features, Actions)
            weights = self.options_W[option]
            bias = self.options_b[option]
        ...
        return action.item() if action.numel() == 1 else action, logp, entropy
...
class SoftOptionCriticFeatures(nn.Module):
    def __init__(self,
                in_features,
                num_actions,
                num_options,
        ...
        super(SoftOptionCriticFeatures, self).__init__()
...
def critic_loss(model, model_prime, data_batch, args):
    ...
    #Incorporate entropy term into the target Q values
    with torch.no_grad():
        _, _, next_entropy = model.get_action(next_states, options)
    alpha = model.alpha.detach()
    soft_next_Q = next_Q_prime[batch_idx, options] + alpha * next_entropy
    soft_max_Q = next_Q_prime.max(dim=-1)[0] + alpha * next_entropy


    # Now we can calculate the update target gt
    gt = rewards + masks * args.gamma * \
        ((1 - next_options_term_prob) * soft_next_Q + next_options_term_prob  * soft_max_Q)
...
def actor_loss(obs, option, logp, entropy, reward, done, next_obs, model, model_prime, args):
    ...
    # Incorporate entropy for soft Actor loss
    alpha = model.alpha.detach()
    # actor-critic policy gradient with entropy regularization
    policy_loss = -logp * (gt.detach() - Q[option]) - alpha * entropy

    alpha_loss = - (model.log_alpha * (entropy - model.target_entropy).detach()).mean()
    actor_loss = termination_loss + policy_loss + alpha_loss
    return actor_loss
